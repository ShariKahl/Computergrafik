...
const float GRUNDHELLIGKEIT = 0.75f;    // Grundhelligkeit der Szene
    ...
    void write_ppm() {
        ...
        for (int j = 0; j < image_height; j++) { 
            for (int i = 0; i < image_width; i++) { 
                ...
                file << static_cast<int>(pixel_color[0] * 255) << " " // Rot-Wert
                     << static_cast<int>(pixel_color[1] * 255) << " " // Grün-Wert
                     << static_cast<int>(pixel_color[2] * 255) << " "; // Blau-Wert
            }
            file << "\n";
        }
    }
...
// Bei mehreren Lichtquellen muss der resultierende diffuse Farbanteil durch die Anzahl Lichtquellen geteilt werden.
    ...
    float lambertian_shading(Vector3df light, Vector3df normal, float kd) {
        Vector3df n = normal;
        n.normalize();
        Vector3df l = light;
        l.normalize();
...
int main() {
    ...
    // Lichtquelle in erhöhter, zentraler Position in der Szene definieren
    Vector3df light = {0.0f, (SZENENKANTE/2) - 0.5f, -TIEFE_MITTELPUNKT};
    ...
    Object objectBottom = Object(sphereBottom, Material({1.0, 1.0, 1.0}, 0.4, 0.0, 0.0));
    Object objectBack = Object(sphereBack, Material({0.9, 0.9, 0.9}, 0.4, 0.3, 0.0));

    Object object1 = Object(sphere1, Material({0.0, 0.0, 0.9}, 0.6, 0.9, 0.0));
    Object object2 = Object(sphere2, Material({0.9, 0.0, 0.0}, 0.4, 0.3, 0.0));
    ...
    for (int x = 0; x < screen.image_width; x++) {
        for (int y = 0; y < screen.image_height; y++) {
            ...
            if(scene.findLightSourcesFromLastHitContext()) {
                pixel_color = ((1 - GRUNDHELLIGKEIT) * scene.lambertian_shading(light - firstIntersection, firstNormal, closestObject->material.shininess) + GRUNDHELLIGKEIT) * pixel_color;
            } else {
